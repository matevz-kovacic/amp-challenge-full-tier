...
# Derived from the pinned organizer validator; only the similarity import/calls
# use RapidFuzz Indel.normalized_similarity. See licenses/organizer-BSD-3-Clause.txt.
...
def _veritfy_max_simularity(sequences: set[str], references: set[str], threshold: float = 0.8) -> None:
    for seq in sequences:
        for ref in references:
            if Indel.normalized_similarity(seq, ref) > threshold:
                raise ValueError(
